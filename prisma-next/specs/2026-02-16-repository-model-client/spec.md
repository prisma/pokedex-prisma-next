...
The name "Collection" was chosen because it represents a _set of model records_ that you progressively refine. You start with all records of a model and narrow it down. It's the thing you interact with most — building up a query piece by piece until you call a terminal method like `all()` to execute it.
...
const postsWithAdminAuthors = await db.posts.include("author", (a) => a.admins()).all();
```
...
type IncludeResultType<Cardinality, Row> = Cardinality extends "1:1" | "N:1"
  ? Row | null
  : Cardinality extends "1:N" | "M:N"
    ? Row[]
    : Row[];
```
...
type CreateInput<TContract, ModelName> = {
  ...
  [K in RequiredCreateFields<TContract, ModelName>]: FieldJsType<TContract, ModelName, K>;
} & {
  // Optional: nullable fields, fields with defaults, auto-generated fields
  [K in OptionalCreateFields<TContract, ModelName>]?: FieldJsType<TContract, ModelName, K>;
};
```
...
```typescript
db.users.where((u) => u.role.eq("admin")).where((u) => u.active.eq(true));
...
db.users.where((u) => u.posts.some((p) => p.comments.some((c) => c.approved.eq(true))));
```
...
// Nested includes
db.users.include("posts", (p) =>
  p.include("comments", (c) => c.where((comment) => comment.approved.eq(true))),
);
```
...
db.users.orderBy([(u) => u.lastName.asc(), (u) => u.firstName.asc()]);

// Chained orderBy appends to the order list
db.users.orderBy((u) => u.lastName.asc()).orderBy((u) => u.firstName.asc());
```
...
const users: UserRow[] = await db.users.where((u) => u.active.eq(true)).all();
```
...
const firstAdmin = await db.users.where((u) => u.role.eq("admin")).find();

// Composes with select() and include()
const user = await db.users.select("name", "email").include("posts").find({ id: 42 });
...
const updated = await db.users.include("posts").where({ id: 42 }).update({ name: "Alice Updated" });
...
const user = await db.users.where({ id: 42 }).update({ name: "Alice Updated" });
// → Promise<Row | null>

// updateAll — update all matches, return them (streamable)
const users = await db.users.where((u) => u.role.eq("guest")).updateAll({ active: false });
// → AsyncIterableResult<Row>

// updateCount — update all matches, return count
const count = await db.users.where((u) => u.role.eq("guest")).updateCount({ active: false });
// → Promise<number>
...
const user = await db.users.create({
  ...
  posts: (p) => p.create([{ title: "First Post" }, { title: "Second Post" }]),
});
```
        ...
        teams: (t) =>
          t.create([
            ...
            },
            {
              name: "Product",
              members: (m) =>
                m.create([{ role: "lead", user: (u) => u.connect({ email: "charlie@acme.com" }) }]),
            },
          ]),
      },
      {
          ...
          t.create([
            {
              name: "Growth",
              members: (m) =>
                m.create([{ role: "lead", user: (u) => u.connect({ email: "diana@acme.com" }) }]),
            },
          ]),
      },
    ]),
...
```typescript
const roleStats = await db.users.groupBy("role").aggregate((a) => ({ count: a.count() }));
// Type: Array<{ role: string; count: number }>

const deptSalaries = await db.employees.groupBy("department").aggregate((a) => ({
...
Include refinement callbacks do **not** execute queries (they have no `all()` / `find()` terminals). Instead, they return a _query fragment_ describing how the include should be loaded.
...
```typescript
const postsWithCommentCount = await db.posts.include("comments", (c) => c.count()).all();
...
class UserCollection extends Collection<Contract, "User"> {
  ...
  recentlyCreated(since: Date) {
    return this.where((u) => u.createdAt.gte(since)).orderBy((u) => u.createdAt.desc());
  }
}

// These compose naturally:
const recentAdmins = await db.users.admins().recentlyCreated(lastWeek).take(10).all();
```
...
```typescript
import { orm, Collection, and, or, not, all } from "@prisma-next/sql-orm-client";
```
...
class PostCollection extends Collection<Contract, "Post"> {
  ...
  withComments() {
    return this.include("comments", (c) =>
      c.where((comment) => comment.approved.eq(true)).orderBy((comment) => comment.createdAt.asc()),
    );
  }
}
...
async function main(db: ReturnType<typeof createClient>) {
  ...
  const updated = await db.users.where({ id: 42 }).update({ name: "Alice Updated" });
  ...
  // Aggregation + GroupBy
  const activeStats = await db.users.active().aggregate((a) => ({ count: a.count() }));

  const roleCounts = await db.users.groupBy("role").aggregate((a) => ({ count: a.count() }));
