...
const user = await db.users.where((u) => u.active.eq(true)).find({ email: "alice@example.com" });
...
const users = await db.users
  .where((u) =>
    and(or(u.email.ilike("%@example.com"), u.role.eq("admin")), not(u.active.eq(false))),
  )
  .all();
```
...
```typescript
const users = await db.users.where((u) => u.posts.some((p) => p.published.eq(true))).all();
```
...
```typescript
const users = await db.users.include("posts", (p) => p.include("comments")).all();
```
...
```typescript
const users = await db.users.orderBy([(u) => u.lastName.asc(), (u) => u.firstName.asc()]).all();
```
...
const usersWithRecentPosts = await db.users.include("posts", (p) => p.published().recent(5)).all();
```
...
function buildFilters(params: URLSearchParams): WhereExpr[] {
  ...
  if (params.has("role")) filters.push(/* construct WhereExpr for role */);
  if (params.has("minAge")) filters.push(/* construct WhereExpr for age >= minAge */);
  return filters;
}

// Use externally-built AST nodes through the callback overload
const externalFilters = buildFilters(req.query);
const users = await db.users.where(() => and(...externalFilters)).all();
```
  ...
  .include("items", (item) =>
    item.include("product", (prod) =>
      prod
        ...
        .include("seller", (s) =>
          s.include("rating").include("badges", (b) => b.where({ active: true })),
        ),
    ),
  )
          ...
          teams: {
            create: [
              ...
              },
              {
                name: "Product",
                members: {
                  create: [{ user: { connect: { email: "charlie@acme.com" } }, role: "lead" }],
                },
              },
            ],
          },
            ...
            create: [
              {
                name: "Growth",
                members: {
                  create: [{ user: { connect: { email: "diana@acme.com" } }, role: "lead" }],
                },
              },
            ],
          },
...
const org = await db.organizations
  ...
  .include("departments", (d) =>
    d.include("teams", (t) => t.include("members", (m) => m.include("user"))),
  );
```
        ...
        d.teams.some((t) =>
          t.members.some((m) =>
            and(
              m.role.eq("lead"),
              m.user.some((u) =>
                u.posts.some((p) => and(p.published.eq(true), p.likes.count().gte(100))),
              ),
            ),
          ),
        ),
