...
describe("planInvalid", () => {
  ...
  it("creates error with details", () => {
    const error = planInvalid("Test error", { key: "value" });
    expect(error.details).toEqual({ key: "value" });
  });

  it("creates error with hints", () => {
    const error = planInvalid("Test error", undefined, ["hint1", "hint2"]);
    expect(error.hints).toEqual(["hint1", "hint2"]);
  });

  it("creates error with docs", () => {
    const error = planInvalid("Test error", undefined, undefined, ["doc1", "doc2"]);
    expect(error.docs).toEqual(["doc1", "doc2"]);
  });

  it("creates error with all properties", () => {
    const error = planInvalid("Test error", { key: "value" }, ["hint1"], ["doc1"]);
