...
function attachGroupingKey<Row>(plan: ExecutionPlan<Row>, groupingKey: string): ExecutionPlan<Row> {
...
- **Use SQL fingerprint only**: fingerprints group _identical SQL_, not “belongs to the same user operation”, and cannot represent multi-statement workflows.
