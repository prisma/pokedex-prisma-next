...
```ts
export interface SqlRuntimeAdapter<TTarget extends string = string> extends RuntimeAdapterInstance<
  "sql",
  TTarget
> {
...
The execution stack is a _target-bound_ bundle of components:
...
- The stack is about _wiring target-bound components_. “Family” is a broader semantic layer that defines how to interpret and execute plans for a given family (e.g., SQL), and it often exists independently of any one target.
