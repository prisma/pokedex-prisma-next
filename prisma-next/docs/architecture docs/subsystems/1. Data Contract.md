...
**Responsibilities:** models/relations; storage (tables, columns, indexes, FK/PK); type identifiers and branded types; capability flags; contract hashing; authoring via PSL or a TS builder; deterministic emission to `contract.json` and `.d.ts` types.
...
- **contract.json:** canonical JSON describing models, storage, capabilities, and metadata
...
```json
{
  ...
  "profileHash": "sha256:…",
  "capabilities": {
    "postgres": { "jsonb": true, "lateral": true, "jsonAgg": true, "vector": false }
  },
  "models": {
    "User": {
      "storage": { "table": "user" },
        ...
        "indexes": [
          {
            "ref": {
              "kind": "index",
              "table": "public.post",
              "name": "post_published_created_at_idx"
            },
...
## Read-Only Sources in the Contract

Pack-owned blocks like views can be projected as read-only sources to the DSL to enable type-safe querying while enforcing immutability.
...
See [ADR 127 — Views as extension-owned read-only sources](../adrs/ADR%20127%20-%20Views%20as%20extension-owned%20read-only%20sources.md) for the full design of views as read-only sources, including migrations, materialization, and adapter lowering.
