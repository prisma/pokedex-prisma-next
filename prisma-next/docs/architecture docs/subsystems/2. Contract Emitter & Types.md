...
export type CodecTypes = Pick<PgCore, "core/string@1" | "core/int@1" | "core/iso-datetime@1">;
```
...
- `prisma-next contract emit` — parse/build → normalize → validate → canonicalize → hash → write artifacts
