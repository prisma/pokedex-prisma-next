...
The runtime is the executable core of Prisma Next. Its job is to execute queries and deliver tight feedback loops as it does so. Like the other subsystems, the runtime is simply an orchestrator of composable, primitive components responsible for execution, verification and the plugin system.

Every query is compiled into a Plan, and every Plan passes through verification and a deterministic plugin pipeline before execution. This makes behavior explicit and problems easy to diagnose.
...
The minimal plugin system keeps logic out of the core and makes behavior composable. First‑party and community plugins can implement lints, budgets, and telemetry. Every Plan is presented to plugins before execution so policies and checks run consistently.
...
- Optionally, the runtime or plugins may check that `plan.meta.storageHash` matches the loaded contract for diagnostic clarity.
...
1. Create
   ...
   - `createRuntime({ stackInstance, context, driver: stackInstance.driver, verify, plugins })` - Creates runtime with now-bound driver.
