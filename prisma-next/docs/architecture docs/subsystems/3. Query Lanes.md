...
Query Lanes define the authoring surfaces that compile to a single, unified Plan model. Each lane offers different ergonomics (DSL, raw SQL with annotations, ORM macros, TypedSQL, Kysely), but all produce immutable Plans that the same runtime executes with consistent verification and guardrails.
...
- Kysely lane: Uses Kysely query builder; transforms `compiledQuery.query` (Kysely AST) into PN `QueryAst` via `transformKyselyToPnAst`, attaches `plan.ast`, and populates `meta.refs`, `meta.paramDescriptors`. Pre-transform guardrails reject unqualified refs and ambiguous `selectAll` in multi-table scope. See [ADR 162](../adrs/ADR%20162%20-%20Kysely%20lane%20emits%20PN%20SQL%20AST.md) and `@prisma-next/integration-kysely`.
...
export interface Plan<Row = unknown> {
  ...
  meta: {
    ...
    storageHash: string; // from contract.json
    profileHash?: string;
    lane: "dsl" | "orm" | "raw-sql" | "typed-sql" | "kysely";
    createdAt: string;
...
const plan = o
  .user()
  .include.posts((child) => child.select({ id: t.post.id, title: t.post.title }).limit(10))
  .select({ id: t.user.id, email: t.user.email })
  .findMany();
...
Extension packs and adapters describe `operations[]` in their manifest, binding a `typeId` to method names, argument/return specs, lowering templates, and optional capability keys. The SQL lane assembles these manifests into an `OperationRegistry` and dynamically attaches the matching methods to each `ColumnBuilder` whose `columnMeta.type` matches a registered `typeId` (`packages/2-sql/4-lanes/relational-core/src/operations-registry.ts`). Calling a registered method emits an `OperationExpr` with deterministic lowering, keeps `meta.refs` accurate, and can expose further operations on the result type. Capability-gated operations only become available when the contract literals declare the required keys, otherwise attempts surface as compile-time errors (when literal) or `PLAN.UNSUPPORTED` diagnostics, keeping operator wiring manifest-driven and safe.
...
1. **compile**
   - For DSL, ORM, and Kysely (with attached AST), lower AST → SQL with adapter and fill refs and projection
   - For raw and TypedSQL, use provided sql, params, and annotations
2. **beforeExecute**
   - Verify `storageHash` and pinned profile, apply lints and budgets, resolve policy config
3. **execute**
...
- Alternate runtimes are allowed — they must verify `storageHash`, respect immutability, and document hook semantics
...
- [ADR 162 — Kysely lane emits PN SQL AST](../adrs/ADR%20162%20-%20Kysely%20lane%20emits%20PN%20SQL%20AST.md)
...
- **ADR 162** — Kysely lane emits PN SQL AST
  - Transform Kysely compiledQuery.query → PN QueryAst; attach plan.ast, meta.refs, meta.paramDescriptors
  - Pre-transform guardrails for qualified refs and unambiguous selectAll; AST-first lints in SQL domain
  - Why an ADR: Kysely as authoring lane with lane-agnostic plugin inspection
