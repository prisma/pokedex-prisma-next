...
- **Composition helpers:** live in extensions and compose runtime + target descriptors into app-facing entrypoints (for example, `@prisma-next/postgres` exposing SQL and Kysely lanes from one helper)
...
- **Migrations (DDL):** The migration runner uses adapter-provided DDL operations and `txSemantics` with compensations or fallback when `transactionalDDL` is unavailable (see [ADR 037](../adrs/ADR%20037%20-%20Transactional%20DDL%20Fallback.md) and [ADR 044](../adrs/ADR%20044%20-%20Pre%20&%20post%20check%20vocabulary%20v1.md))
...
- Provide op executors for the migration runner with txSemantics and compensations
  See [ADR 037](../adrs/ADR%20037%20-%20Transactional%20DDL%20Fallback.md) and [ADR 044](../adrs/ADR%20044%20-%20Pre%20%26%20post%20check%20vocabulary%20v1.md)
...
```json
{
  ...
  "capabilities": {
    "mongo": { "transactions": true, "stages": ["match", "lookup", "project", "limit", "sort"] }
  }
}
```
...
Migration ops are document-specific but follow the same edge model (see [ADR 028](../adrs/ADR%20028%20-%20Migration%20Structure%20%26%20Operations.md) and [ADR 044](../adrs/ADR%20044%20-%20Pre%20%26%20post%20check%20vocabulary%20v1.md)):
...
4. **Pinned profile validation**: Runtime validates environment satisfies the contract‑pinned capability profile; no `profileHash` recomputation (see [ADR 004](../adrs/ADR%20004%20-%20Core%20Hash%20vs%20Profile%20Hash.md))
