...
2. **Layers** (responsibility-based layers). Layers express dependency _direction_: packages may depend on peers in the same layer (lateral relationships) and on layers closer to core (downward), but never "upward."
