...
- **Phase 1 (merge what we have):** keep the current runtime-attached integration, but make it _usable as Prisma Next_ by producing **Prisma Next `QueryAst`** in query plans (not just SQL strings) so adapters/plugins/guardrails can inspect and enforce behavior.
